...
<table width="100%">
    <tr>
        <td width="50%">
            <input type="button" class="btn btn-default" onclick="location.href='/comunicado/crear/proyectoId/{$proyecto.id}/'" value="{$Tr->_("Agregar Comunicado")}" title="{$Tr->_("Agregar Comunicado")}">
        </td>
...
<table border="0"  width="100%" class="table table-hover">
  {foreach $rows as $row}
        ...
        <tr>
            ...
            <td>
                <input type="button" class="btn btn-default" onclick="location.href='/comunicado/actualizar/id/{$row.id}/'" value="{$Tr->_("editar")|capitalize}" title="{$Tr->_("editar_comunicado")} ({$row.nombre})">
                <input type="button" class="btn btn-default" onclick="location.href='/mail/crear/proyectoId/{$proyecto.id}/param/Comunicado/paramId/{$row.id}'" value="{$Tr->_("notificar")|capitalize}" title="{$Tr->_("notificar")} ({$row.nombre})">
                <input type="button" class="btn btn-default" onclick="return confirmar('/comunicado/eliminar/id/{$row.id}/', 'Seguro desea eliminar el comunicado ({$row.nombre})?')" value="{$Tr->_("eliminar")|capitalize}" title="{$Tr->_("eliminar_usuario")} ({$row.nombre})">
            </td>
        </tr>
  {/foreach}</table>
